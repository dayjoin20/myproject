...
const Switchbutton = () => {
    ...
    if (switch2Enabled) {
      ...
      const callback = (snapshot) => {
        ...
        if (data) {
          ...
          setAverageMoisture(average); // Update state with the new average moisture value
      ...
      setAverageMoisture(null); // Clear average moisture when switch2 is disabled
  ...
  return (
    <div>
      {switch2Enabled && averageMoisture !== null && (
        <div className='average-moisture'>
          <label style={{ color: 'white', marginRight: '5px' }}>ค่าเฉลี่ยความชื้นในดิน: </label>
          <span style={{ color: averageMoisture <= 75 ? 'orange' : averageMoisture >= 80 ? 'green' : 'black' }}>
